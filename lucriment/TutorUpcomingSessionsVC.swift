...
class TutorUpcomingSessionsVC: UIViewController {
	...
	var parentVC: TutorSessionsController! // to show the current session
	...
	override func viewWillAppear(_ animated: Bool) {
		...
		if ReachabilityManager.shared().reachabilityStatus == .notReachable {
			...
			DispatchQueue.main.async {
				DatabaseManager.shared.downloadUpcomingSessions(for: .tutor, id: UserManager.shared.id) { (sessions, error) in
					if let error = error {
...
extension TutorUpcomingSessionsVC: UITableViewDataSource, UITableViewDelegate {
	...
	func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		if section == 0 {
			...
			cell.nameLabel.text = self.sessionRequests[row].studentName
		...
		let cell = self.tableView.dequeueReusableCell(withIdentifier: "SessionCell")! as! SessionCell
		cell.nameLabel.text = self.confirmedSessions[row].studentName
	...
	func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
		...
		let mainStoryboard: UIStoryboard = UIStoryboard(name: "TutorSessions", bundle: nil)
		...
		if section == 0 {
			...
			let vc = mainStoryboard.instantiateViewController(withIdentifier: "TutorUpcomingSessionRequestDetailsVC") as! TutorUpcomingSessionRequestDetailsVC
			vc.sessionRequest = self.sessionRequests[row]
			...
			let vc = mainStoryboard.instantiateViewController(withIdentifier: "TutorUpcomingSessionDetailsVC") as! TutorUpcomingSessionDetailsVC
