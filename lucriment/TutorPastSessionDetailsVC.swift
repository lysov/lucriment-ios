//
//  TutorPastSessionDetailsVC.swift
//  lucriment
//
...
class TutorPastSessionDetailsVC: UITableViewController {
	...
	override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
		if segue.identifier == "RateStudentController" {
			let navigationController = segue.destination as! UINavigationController
			let vc = navigationController.viewControllers.first as! RateStudentVC
			vc.sessionDetailsVC = self
		}
	}
}

extension TutorPastSessionDetailsVC {
	
	// MARK: - Table view data source
	
	override func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
		
		var locationIndexPath = IndexPath(row: 4, section: 0)
		
		if locationIndexPath == indexPath {
			return UITableViewAutomaticDimension
		}
		
		locationIndexPath = IndexPath(row: 0, section: 1)
		
		if locationIndexPath == indexPath, self.session.studentReview != nil {
			return 0
		}
