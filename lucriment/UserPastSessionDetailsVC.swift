//
//  UserPastSessionDetailsVC.swift
//  lucriment
//
...
class UserPastSessionDetailsVC: UITableViewController {
	...
	override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
		if segue.identifier == "RateTutorController" {
			let navigationController = segue.destination as! UINavigationController
			let vc = navigationController.viewControllers.first as! RateTutorVC
			vc.sessionDetailsVC = self
		}
	}
}

extension UserPastSessionDetailsVC {
	...
	override func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
		...
		if locationIndexPath == indexPath, self.session.tutorReview != nil {
			return 0
		}
