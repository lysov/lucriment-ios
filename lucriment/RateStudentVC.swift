...
class RateStudentVC: UIViewController {

	@IBOutlet weak var ratingCosmosView: CosmosView!
	@IBOutlet weak var textView: UITextView!
	var sessionDetailsVC: TutorPastSessionDetailsVC!
	var text: String!
	...
	@IBAction func doneButtonDidPress(_ sender: Any) {
		...
		if let text = self.text {
			let author = self.sessionDetailsVC.session.tutorName
			let authorId = self.sessionDetailsVC.session.tutorId
			...
			DatabaseManager.shared.rate(.student, session: self.sessionDetailsVC.session, review: review.toDictionary()) { (error) in
				self.sessionDetailsVC.session.studentReview = review
...
// textView methods
extension RateStudentVC: UITextViewDelegate {
