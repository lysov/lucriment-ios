...
class TutorPastSessionsVC: UIViewController {
	...
	override func viewWillAppear(_ animated: Bool) {
		...
		if ReachabilityManager.shared().reachabilityStatus == .notReachable {
			...
			DispatchQueue.main.async {
				DatabaseManager.shared.downloadPastSessions(for: .tutor, id: UserManager.shared.id) { (sessions, error) in
					if let error = error {
						...
						for session in sessions {
							if session.sessionDeclined == true && session.sessionCancelled == false {
								...
								let sessionDate = Int(session.time.from.timeIntervalSince1970 * 1_000)
								if date > sessionDate {
...
extension TutorPastSessionsVC: UITableViewDataSource, UITableViewDelegate {
	...
	func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		if section == 0 {
			...
			cell.nameLabel.text = self.sessionHistory[row].studentName
		...
		if section == 1 {
			...
			let cell = self.tableView.dequeueReusableCell(withIdentifier: "SessionCell")! as! SessionCell
			cell.nameLabel.text = self.declinedSessions[row].studentName
		...
		let cell = self.tableView.dequeueReusableCell(withIdentifier: "SessionCell")! as! SessionCell
		cell.nameLabel.text = self.cancelledSessions[row].studentName
	...
	func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
		...
		if section == 0 {
			...
			let vc = mainStoryboard.instantiateViewController(withIdentifier: "TutorPastSessionDetailsVC") as! TutorPastSessionDetailsVC
			...
			let vc = mainStoryboard.instantiateViewController(withIdentifier: "TutorPastCancelledSessionDetailsVC") as! TutorPastCancelledSessionDetailsVC
