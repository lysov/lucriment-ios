...
class TutorSessionsController: UIViewController {
	...
	func present(_ session: Session) {
		...
		// user name
		self.currentSessionUserLabel.text = session.studentName
	...
	override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
		if (segue.identifier == "TutorUpcomingSessionsVC") {
			let vc = segue.destination as! TutorUpcomingSessionsVC
			vc.parentVC = self
		}
	}
}
